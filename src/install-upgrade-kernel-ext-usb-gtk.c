/*
install-upgrade-kernel-ext-usb-gtk.c from Slackel <http://www.slackel.gr>
...
void do_action (gboolean doit) {
	...
	if (strlen(fstype) == 0) {
		...
		gtk_combo_box_get_active_iter(listwidget, &iter);
		list = (GtkListStore *) gtk_combo_box_get_model(listwidget);
		gtk_tree_model_get((GtkTreeModel *) list, &iter, 0, &secs, -1);
		if (strlen(secs) == 0) {
			g_free(secs);
			secs = g_strdup("10");
			}
	...
	if (doit) {
		commandline = g_strdup_printf("install-upgrade-kernel-ext-usb %s %s %s %s\n", kernel_action_mode, kernel_type, secs, fstype );
	}    
